"""
CryptoMind — Signaux eToro (actions + or) à partir d'indicator_engine / scoring_engine.
...
from app.services import indicator_engine, scoring_engine
from app.utils.logging_utils import get_logger
from etoro.models import Side, Signal

log = get_logger("etoro_signals")
...
async def build_signal(agent, symbol: str) -> Signal | None:
    ...
    if inst is None:
        log.warning("etoro_signal_unknown_symbol", symbol=symbol)
        return None
    candles = await agent.service.get_candles(inst.instrument_id, CANDLE_INTERVAL, CANDLE_COUNT)
    if len(candles) < MIN_CANDLES:
        log.warning("etoro_signal_not_enough_candles", symbol=symbol, candles=len(candles))
        return None
    ...
    async def _one(symbol: str) -> Signal | None:
        async with sem:
            try:
                ...
                log.error("etoro_signal_error", symbol=symbol, error=str(e))
                return None
